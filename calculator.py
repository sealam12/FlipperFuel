...
class TargetCalculator:
    ...
    def total_race_laps(self):
        ...
        avg_lap_time = self.data.lap_data.avg_lap_time
...
class LiveCalculator:
    def __init__(self, race_data):
        ...
        self.fuel_remaining = self.data.car_info.max_fuel
        self.minutes_remaining = self.data.race_info.race_length

        self.completed_stints = 0
        self.avg_lap_time = self.data.lap_data.avg_lap_time
        self.avg_lap_fuel = self.data.lap_data.avg_lap_fuel
    ...
    def total_race_fuel(self):
        total_race_laps = self.total_race_laps()
        total_race_fuel = total_race_laps * self.avg_lap_fuel

        return total_race_fuel
        
    def remaining_race_laps(self):
        seconds_remaining = self.minutes_remaining*60
        avg_lap_time = self.data.lap_data.avg_lap_time
    ...
    def remaining_race_fuel(self):
        ...
        remaining_race_fuel = remaining_race_laps * self.avg_lap_fuel

        return remaining_race_fuel

    def projected_stints_remaining(self):
        remaining_laps = self.remaining_race_laps()
        required_fuel = remaining_laps * self.avg_lap_fuel
    ...
    def projected_stints_total(self):
        return self.projected_stints_remaining() + self.completed_stints

    def current_stint_laps_remaining(self):
        remaining_fuel = self.fuel_remaining
        remaining_laps = remaining_fuel / self.avg_lap_fuel

        return remaining_laps
    
    def current_stint_time_remaining(self):
        remaining_laps = self.current_stint_laps_remaining()
        remaining_time = (remaining_laps * self.avg_lap_time) / 60

        return remaining_time
    
    def stint_laps_required(self):
        stints_remaining = self.data.targets.stint_target - self.completed_stints
    ...
    def lap_fuel_required(self):
        stints_remaining = self.data.targets.stint_target - self.completed_stints
        fuel_remaining = self.fuel_remaining + ( ( stints_remaining-1 ) * self.data.car_info.max_fuel )
    ...
    def stint_time_required(self):
        ...
        time_required = (laps_required * self.avg_lap_time) / 60
